...
describe('createAdmin Server Action Test', () => {
    let mockDb: any;
    ...
    it('should create admin successfully and return code 0', async () => {
        ...
        // 执行 Action (此时 createAdmin 已被剥离中间件外壳)
    ...
    it('should return code 1 if email already exists', async () => {
        ...
        const result = await createAdmin(SCHEMA, { code: -1, timestamp: 0 }, formData);

        expect(result.code).toBe(1);
        expect(result.message).toBe('This email is already in use');
    });

    it('should return code 2 if passwords do not match', async () => {
        const formData = getMockFormData({ confirmPwd: 'wrong_password' });

        const result = await createAdmin(SCHEMA, { code: -1, timestamp: 0 }, formData);

        expect(result.code).toBe(2);
        expect(result.error?.password).toContain('not match');
    });
